...
static void LimitTextureSize(int *w_upscale, int *h_upscale)
{
    ...
    {
        debugPrint("CreateUpscaledTexture: SDL_GetRendererInfo() call failed: %s",
                SDL_GetError());
    }*/
    ...
    {
        debugPrint("CreateUpscaledTexture: Can't create a texture big enough for "
    ...
    {
        debugPrint("CreateUpscaledTexture: max_scaling_buffer_pixels too small "
    ...
    {
        debugPrint("CreateUpscaledTexture: Limited texture size to %dx%d "
...
static void CreateUpscaledTexture(bool force)
{
    ...
    {
        debugPrint("Failed to get renderer output size: %s", SDL_GetError());
    }
...
{
    tonccpy(scr, I_VideoBuffer, SCREENWIDTH*SCREENHEIGHT*sizeof(*scr));
}
...
static void SetVideoMode(void)
{
    ...
    if (screen == NULL)
    {
        ...
        if (screen == NULL)
        {
            debugPrint("Error creating window for video startup: %s",
            SDL_GetError());
        }
    ...
    {
        debugPrint("Could not get display mode for video display #%d: %s",
    ...
    if (renderer == NULL)
    {
        debugPrint("Error creating renderer for screen window: %s",
                SDL_GetError());
    }
...
void I_InitGraphics(uint8_t *pal)
{
    ...
    {
        debugPrint("Failed to initialize video: %s", SDL_GetError());
    }

...
)
{
    ...
    if (OBJS_GetAmt(S_ENERGY) <= 0)
        debugPrint("RAP_LoadPLayer() - Loaded DEAD player");
...
)
{
    ...
    if (filepos == -1)
        debugPrint("RAP_Save() ERR: Try to Save Invalid Player");
    
    if (OBJS_GetAmt(S_ENERGY) <= 0)
        debugPrint("RAP_Save() ERR: Try to save Dead player");
...
)
{
    ...
    if (!gameflag[cur_game])
        debugPrint("Loading Invalid map game %d", cur_game);
    ...
    if (map_item == -1)
        debugPrint("RAP_LoadMap() - Invalid MAP.(%s)", temp);
